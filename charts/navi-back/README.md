...
| naviback.forecast_host | string | `""` | URL of forecast server  |
...
```
2. Создать файл dammam_values.yaml со следующим содержимым:
```
...
service:
  annotations:
    ...
    router.deis.io/domains: dammam-cr
...
```
helm upgrade --install test-dammam-cr . -f dammam_values.yaml
```
4. Отправить POST-запрос на pod(в примере через port forwarding kubernetes):
```
kubectl port-forward test-dammam-cr-navi-back-6864944c7-vrpns 7777:8080
```
