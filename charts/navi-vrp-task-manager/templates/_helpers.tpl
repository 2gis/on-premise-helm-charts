...
{{- /* Merge .kafka.properties and .kafka.fileProperties dictionaries.
     ...
     */ -}}
{{- define "navi-vrp-task-manager.kafkaProperties" -}}
  {{- $ctx := . -}}
...
{{- /* Translate properties into `env` construction as in containers:
     ...
     */ -}}
{{- define "navi-vrp-task-manager.kafkaPropertiesEnv" -}}
  {{- $ctx := . -}}
  {{- $kafkaProperties := get (fromYaml (include "navi-vrp-task-manager.kafkaProperties" $ctx)) "ret" -}}
  {{- $env := list -}}
