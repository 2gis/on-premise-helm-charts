...
- Константы или переменные, которые никогда не меняются при типовом использовании сервиса, следует скрывать из `README.md` при помощи тэга `@skip`.
  ...
  Обратите внимание, что из-за бага в генераторе описание не может начинаться со ссылки (любая конструкция в квадратных скобках в начале описания будет принята за декларацию типа). Формулируйте описания настроек и секций так, чтобы ссылки были не в начале.
...
- Блоки настроек, связанные с конкретными сервисами 2GIS, должны быть названы так же, как этот сервис. Например: не `catalog`, `keys`.

- Блоки настроек, связанные с другими сервисами, должны быть названы так же, как этот сервис. Например: не `storage`, а `postgres` или `s3`.
